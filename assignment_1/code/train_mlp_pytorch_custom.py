...
def accuracy(predictions: torch.Tensor, targets: torch.Tensor):
    ...
    acc = float(torch.sum(torch.argmax(predictions, dim=1) == targets)) / float(targets.shape[0])
...
def train():
    ...
    for step in range(FLAGS.max_steps):
        ...
        x = torch.tensor(x.reshape(FLAGS.batch_size, -1), dtype=torch.float)
        ...
        if step % FLAGS.eval_freq == 0:
            ...
            with torch.no_grad():
                ...
                loss_test = criterion.forward(y_test, t_test)
